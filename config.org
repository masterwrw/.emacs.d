...
** Move to the begging of line ignore tab
#+BEGIN_SRC emacs-lisp
(defun eye/move-beginniing-of-line ()
  "移动到行首时，忽略最前面的空白字符"
  (interactive)
  (move-beginning-of-line 1)
  (forward-word 1)
  (backward-word 1))
(global-set-key (kbd "C-a") 'eye/move-beginniing-of-line)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun set-tab-width-hook ()
    ...
    (setq c-default-style "stroustrup") ;; 大括号缩进位置，https://en.wikipedia.org/wiki/Indentation_style
    (setq tab-stop-list ()))
...
(defun eye/cpp-help ()
  ...
  (let ((url "http://zh.cppreference.com/mwiki/index.php?search="))
    (setq url (concat url (read-string "Query cpp document: ")))
...
(defun eye/qt5-help ()
  ...
  (let ((url "http://doc.qt.io/qt-5/search-results.html?q="))
    (setq url (concat url (read-string "Query Qt5 document: ")))
    (browse-url-default-browser url)))
...
(defun eye/python-help ()
  ...
  (let ((url "https://docs.python.org/3.5/search.html?q="))
    (setq url (concat url (read-string "Query python document: ")))
    (browse-url-default-browser url)))
#+END_SRC
...
(use-package hydra
  ...
  (defhydra hydra-programming (:color red :exit t)
    ...
    ("c" hs-hide-all "hide all block")
    ("s" hs-show-all "show all block")
    ("f" eye/cpp-help "cpp doc")
